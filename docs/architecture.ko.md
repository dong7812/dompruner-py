...
`query`가 주어지면 추출된 섹션을 점수화하여 랭킹합니다. 표준 BM25에서 세 가지를 조정합니다:
...
**제로 스코어 폴백:** 쿼리 키워드가 문서 어디에도 매칭되지 않으면(BM25 최대값 = 0), 빈 결과 대신 정제된 전체 콘텐츠를 반환합니다. 이 경우 `bm25_confidence`는 `None`으로 설정됩니다.
